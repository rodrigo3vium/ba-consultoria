  ...
  return (
    <PropostaLayout cliente="Clínica Supreme" projeto="Arquitetura de Receita" data="Março 2026">
        ...
        <div className="rounded-2xl border border-white/[0.09] bg-saas-card p-5">
          <div className="flex flex-wrap gap-8">
            {[
              ...
              <div key={m.label}>
                <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-saas-faint mb-1">{m.label}</p>
                <p className="font-bold text-saas-ink text-base leading-tight">{m.value}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {mentors.map((m) => (
            <StratCard key={m.name} className="text-center p-6">
              <div className="w-16 h-16 overflow-hidden rounded-full border border-white/[0.10] mx-auto mb-4">
                <img
                  loading="lazy"
                  src={m.photo}
                  alt={m.name}
              ...
              <div className="font-mono text-[10px] uppercase tracking-[0.14em] text-saas-cyan mb-2">{m.role}</div>
      ...
      <div className="border-t border-white/[0.06] py-16 space-y-8">
        ...
        <div className="rounded-2xl border border-white/[0.09] bg-saas-card p-8 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {diagnosticCards.map((c) => (
            <StratCard key={c.title}>
              <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-saas-violet mb-3">{c.flag}</p>
              <h3 className="font-bold text-saas-ink text-base leading-tight mb-3">{c.title}</h3>
              <p className="text-saas-body text-xs leading-relaxed">{c.text}</p>
            </StratCard>
          ))}
        </div>
      </div>
      ...
      <div className="border-t border-white/[0.06] py-16 space-y-8">
        ...
        <div className="rounded-2xl border border-white/[0.09] bg-saas-card overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr>
                <th className="p-4 text-left font-mono text-[10px] uppercase tracking-[0.14em] text-saas-muted border-b border-white/[0.09]">Dimensão</th>
                <th className="p-4 text-left font-mono text-[10px] uppercase tracking-[0.14em] text-saas-muted border-b border-white/[0.09]">Hoje</th>
                <th className="p-4 text-left font-mono text-[10px] uppercase tracking-[0.14em] text-saas-muted border-b border-white/[0.09]">Com a implementação</th>
              </tr>
            </thead>
            <tbody>
              {comparisonRows.map((r) => (
                <tr key={r.dim} className="border-b border-white/[0.06] last:border-0">
                  <td className="p-4 text-saas-body text-sm font-medium">{r.dim}</td>
                  <td className="p-4 text-saas-faint text-sm">{r.old}</td>
                  <td className="p-4 text-saas-cyan text-sm font-medium">{r.now}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="space-y-4">
          {phases.map((phase) => (
            <StratCard key={phase.tag}>
              <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-saas-cyan mb-4">{phase.tag}</p>
              <h3 className="font-bold text-saas-ink text-xl tracking-tight leading-tight mb-4">{phase.title}</h3>
              <p className="text-saas-body text-sm leading-relaxed mb-5">{phase.desc}</p>
              ...
              <p className="text-xs text-saas-faint italic">{phase.note}</p>
            </StratCard>
          ))}
        </div>
      </div>
      ...
      <div className="border-t border-white/[0.06] py-16 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <StratCard key={c.title}>
              <p className="font-mono text-[11px] uppercase tracking-[0.14em] text-saas-violet mb-3">✕ {c.title}</p>
              <p className="text-saas-body text-sm leading-relaxed">{c.text}</p>
            </StratCard>
          ))}
        </div>
      </div>
      ...
      <div className="border-t border-white/[0.06] py-16 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <StratCard>
            <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-saas-faint mb-2">Opção 1</p>
            <h3 className="font-bold text-saas-ink text-lg tracking-tight leading-tight mb-3">Consultoria + Direcionamento</h3>
            <p className="text-saas-muted text-xs leading-relaxed mb-5">
              Eu diagnostico, priorizo e direciono. A equipe da Supreme executa com meu acompanhamento.
            </p>
            <div className="mb-1">
              <span className="font-extrabold text-[clamp(36px,4vw,52px)] leading-none bg-gradient-to-r from-saas-cyan to-saas-violet bg-clip-text text-transparent">R$ 5.000</span>
            </div>
            <p className="text-xs text-saas-faint mb-6">por mês · ciclo mínimo de 90 dias</p>
            <ul className="space-y-2">
              {[
                ...
                <li key={item} className="text-saas-body text-xs leading-relaxed flex gap-2">
                  <span className="text-saas-cyan shrink-0">→</span>
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div className="rounded-2xl border border-saas-violet/60 bg-saas-card p-8 relative">
            <p className="absolute top-4 right-4 inline-flex items-center rounded-full bg-gradient-to-r from-saas-cyan to-saas-violet px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-saas-void">Recomendada</p>
            <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-saas-faint mb-2">Opção 2</p>
            <h3 className="font-bold text-saas-ink text-lg tracking-tight leading-tight mb-3">Consultoria + Implementação</h3>
            <p className="text-saas-muted text-xs leading-relaxed mb-5">
              Tudo da Opção 1 — mais eu botando a mão na massa. Construo os protótipos, automações e integrações junto com vocês.
            </p>
            <div className="mb-1">
              <span className="font-extrabold text-[clamp(36px,4vw,52px)] leading-none bg-gradient-to-r from-saas-cyan to-saas-violet bg-clip-text text-transparent">R$ 8.000</span>
            </div>
            <p className="text-xs text-saas-faint mb-6">por mês · ciclo mínimo de 90 dias</p>
            <ul className="space-y-2">
              {[
        ...
        <div className="space-y-2">
          <p className="text-saas-body text-sm text-center max-w-xl mx-auto leading-relaxed">
      ...
      <div className="border-t border-white/[0.06] py-16 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <StratCard key={c.title}>
              <h3 className="font-bold text-saas-ink text-lg tracking-tight leading-tight mb-3">{c.title}</h3>
              <p className="text-saas-body text-sm leading-relaxed">{c.text}</p>
            </StratCard>
          ))}
        </div>
      ...
      <div className="border-t border-white/[0.06] py-16 space-y-8">
        ...
        <div className="max-w-xl mx-auto space-y-0">
          {[
            ...
            <div key={i} className="flex gap-5 items-start py-5 border-b border-white/[0.06] last:border-0 first:border-t first:border-white/[0.06]">
              <span className="font-extrabold text-2xl leading-none shrink-0 pt-0.5 bg-gradient-to-r from-saas-cyan to-saas-violet bg-clip-text text-transparent">
                {String(i + 1).padStart(2, "0")}
              </span>
              <span className="text-saas-body text-sm leading-relaxed">{step}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="border-t border-white/[0.06] py-16 text-center space-y-8">
        <h2 className="font-extrabold text-saas-ink text-[clamp(28px,4vw,44px)] leading-[1.1] tracking-tight max-w-2xl mx-auto">
          A Supreme já tem tudo que é difícil de construir.<br />
          <Accent>Falta a estrutura.</Accent>
        </h2>
