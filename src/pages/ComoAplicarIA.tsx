            ...
            <CarouselContent className="-ml-4">
              {depoimentos.map((dep, idx) => (
                <CarouselItem key={idx} className="pl-4 basis-full md:basis-1/3">
                  <Card className="relative bg-gradient-to-br from-white/[0.08] to-white/[0.02] backdrop-blur-sm border-white/10 rounded-2xl overflow-hidden h-full">
                    <CardContent className="p-5 md:p-6 flex flex-col h-full">
                      <Quote className="w-8 h-8 text-ba-blue-light/30 mb-3 flex-shrink-0" />
                      <p className="text-base md:text-lg text-white/80 italic leading-relaxed mb-4 flex-grow">
                        "{dep.text}"
                      </p>
                      <div className="flex items-center gap-3 mt-auto">
                        <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-gradient-to-br from-ba-blue-light to-ba-orange flex items-center justify-center text-white font-bold text-sm">
                          {dep.author.charAt(0)}
                        </div>
